...
def yto_client_resource(**kwargs):
    ...
    for purpose, purpose_config in kwargs.items():
        resources.append(
            file_resource(path=VEIL_ETC_DIR / 'yto-client-{}.cfg'.format(purpose), content=render_config(
                'yto-client.cfg.j2', api_url=purpose_config.api_url, type=purpose_config.type, client_id=purpose_config.client_id,
