...
def list_writable_tables():
    ...
    writable_tables = get_application_tables()
    return writable_tables
...
def check_table_dependency(component_name, sql):
    ...
    if not component_name:
        return
    check_writable_table_dependency(list_writable_tables(), component_name, sql)
    check_readable_table_dependency(list_readable_tables(), component_name, sql)
...
def check_readable_table_dependency(readable_tables, component_name, sql):
    sql = sql.strip().replace('\n', '').replace('\r', '').replace('\t', '')
