...
def deploy():
    ...
    shell_execute('veil install veil_installer.component_resource?veil.server.supervisor --upgrade-mode={} --install-when-not-installed'.format(UPGRADE_MODE_FAST))
    shell_execute('veil down')
    shell_execute('veil install-server --upgrade-mode={} --install-when-not-installed'.format(UPGRADE_MODE_FAST))
