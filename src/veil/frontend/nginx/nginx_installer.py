...
def nginx_resource(servers, config):
    resources = [
        ...
        directory_resource(path=VEIL_LOG_DIR / 'nginx'),
        ...
        directory_resource(path=VEIL_BUCKET_UPLOADED_FILES_DIR, mode=0770)
    ]
    resources.extend([
        directory_resource(path=VEIL_BUCKET_UPLOADED_FILES_DIR / unicode(i), mode=0770) for i in range(10)
    ])
    return resources
