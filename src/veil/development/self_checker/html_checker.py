...
def check_no_commented_script_or_style_or_link_element(dir):
    ...
    htmls_with_commented_script_or_style_or_link_element = htmls_with_commented_script_element | htmls_with_commented_style_element \
                                                           | htmls_with_commented_link_element
...
def get_html_files_with_content_matching_pattern(dir, pattern, ignore_case):
    files = set()
    try:
        files = shell_execute('grep -rl{} --include="*.html" "{}" .'.format('i' if ignore_case else '', pattern), expected_return_codes=(0, 1),
