...
def iptables_rule_resource(table, rule):
    installed = rule in fabric.api.sudo('iptables-save -t {}'.format(table), warn_only=True)
    ...
    fabric.api.sudo('iptables -t {} -A {}'.format(table, rule), warn_only=True)
    updated_rules = fabric.api.sudo('sudo iptables-save -t {}'.format(table), warn_only=True)
...
def iptables_rule_removed_resource(table, rule):
    ...
    if installed:
        ...
        fabric.api.sudo('iptables -t {} -D {}'.format(table, rule), warn_only=True)


@atomic_installer
def iptables_policy_resource(table, chain, policy):
    installed = '{} {}'.format(chain, policy) in fabric.api.sudo('iptables-save -t {}'.format(table), warn_only=True)
    dry_run_result = get_dry_run_result()
    if dry_run_result is not None:
    ...
    fabric.api.sudo('iptables -t {} -P {} {}'.format(table, chain, policy), warn_only=True)
