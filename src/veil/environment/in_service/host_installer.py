...
def provision_env(provisioning_env, config_dir):
    for provisioning_server_name in list_veil_servers(provisioning_env).keys():
...
def provision_server(provisioning_env, provisioning_server_name, config_dir):
    provisioning_server = get_veil_server(provisioning_env, provisioning_server_name)
    provisioning_server_host = get_veil_server_host(provisioning_env, provisioning_server.hosted_on)
    sequence_no = provisioning_server.sequence_no
    ...
    fabric.api.env.host_string = '{}:{}'.format(provisioning_server_host.ssh_ip, provisioning_server_host.ssh_port)
    with open('{}/host/{}/pass'.format(config_dir, provisioning_server.hosted_on)) as f:
    ...
    fabric.api.env.host_string = '{}@{}:{}22'.format(user_name, provisioning_server_host.ssh_ip, sequence_no)
