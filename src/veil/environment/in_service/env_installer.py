...
def make_rollback_backup(veil_env_name, exclude_code_dir=False, exclude_data_dir=True):
    for host in unique(list_veil_hosts(veil_env_name), id_func=lambda h: h.base_name):
        ...
        with fabric.api.settings(host_string=host.deploys_via):
            fabric.api.sudo('rsync -avh --numeric-ids --delete {} --link-dest={}/ {}/ {}/'.format(' '.join(excludes), source_dir, source_dir,
                rollback_backup_dir))
    ...
    for host in hosts:
        ...
        with fabric.api.settings(host_string=host.deploys_via):
            if fabric.contrib.files.exists(source_dir):
                ...
                fabric.api.sudo('rsync -avh --numeric-ids --delete --link-dest={}/ {}/ {}/'.format(rollback_backup_dir, source_dir, left_over_dir))
            fabric.api.sudo('rsync -avh --numeric-ids --delete {}/ {}/'.format(rollback_backup_dir, source_dir))
