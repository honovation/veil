...
def elk_resource(config):
    ...
    return [
        ...
        directory_resource(path=VEIL_ETC_DIR / 'elasticsearch', owner=CURRENT_USER, group='elasticsearch'),
        directory_resource(path=VEIL_ETC_DIR / 'elasticsearch/scripts', owner=CURRENT_USER, group='elasticsearch'),
        ...
        file_resource(path=VEIL_ETC_DIR / 'elasticsearch/elasticsearch.yml',
                      content=render_config('elasticsearch.yml.j2', elasticsearch_cluster=VEIL_ENV.name, **config), owner=CURRENT_USER, group='elasticsearch'),
